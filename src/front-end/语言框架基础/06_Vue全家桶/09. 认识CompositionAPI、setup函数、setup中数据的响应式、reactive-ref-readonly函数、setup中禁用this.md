...
但如果将一个新的 ref 赋值给一个关联了已有 ref 的属性，那么它会替换掉旧的 ref：
...
> 当 ref 作为数组或 Map 这种原生集合类型的元素被访问时，不会自动解包。  
> 不同于响应式对象，即使是在访问 reactive 包裹调用的响应式数组或 响应式Map 时，ref 的值也不会自动解包。

```js
const books = [ref('0')]
books[0].value // 必须写 .value

const books2 = reactive([ref('0')])
books2[0].value // 必须写 .value

const map = new Map([['count', ref(0)]])
map.get('count').value // 必须写 .value

const map2 = reactive(new Map([['count', ref(0)]]))
map2.get('count').value // 必须写 .value
```

- **浅层响应式对象**

> 只有当 ref 嵌套在一个深层响应式对象内时，才会发生 ref 解包。  
> 当其作为[浅层响应式对象](https://cn.vuejs.org/api/reactivity-advanced.html#shallowreactive)的属性被访问时不会解包。
...
> 当 ref 在模板中作为顶层属性被访问时会自动解包，但在 setup() 内部，它依然是一个 ref 引用，需要使用 `ref.value` 的方式对其进行操作。

```vue
<template>
  <div>
    <!-- 在 template 中作为顶层属性访问 ref 时, Vue会自动对其进行解包(取出.value) -->
    <h2>当前计数: {{ counter }}</h2>
    <button @click="increment">+1</button>
    <!-- 会自动解包，作为顶层属性访问的 -->
    <button @click="counter++">+1</button>
    <!-- 不会自动解包，非顶层属性访问，需要写.value -->
    <button @click="info.counter++">+1</button> 
    <!-- 会自动解包，符合插值表达式特殊规则，即插值表达式最终计算值是 ref 时，不需要写.value -->
...
export default {
  setup() {
    const counter = ref(0)
    function increment() {
      counter.value++ // setup 内部需要 .value
    }
    // ref是浅层解包
    const info = {
      counter
