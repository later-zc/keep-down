...
- **默认不会影响到当前组件的 `<slot/>` 中渲染的内容（Vue3）**：默认情况下，作用域样式不会影响到当前组件 `<slot>` 渲染出来的内容（插槽默认填充内容除外），因为插槽中的元素被认为是父组件所持有并传递进来的。
...
###  总结
::: tip 总结
1. Vue2 和 Vue3 中，父组件传递给子组件插槽的每个元素，编译后都会携带上父组件作用域标识属性符。

2. Vue2 中 子组件 scoped 样式能作用于父组件传递过来的插槽元素，而 Vue3 中则不能作用。
原因是 Vue3 中，父组件传递给子组件插槽的元素，编译后不会携带子组件作用域标识符属性（data-v-child），而是携带一个插槽元素标识符属性（如 data-v-child-s）。

3. Vue3 中新增插槽选择器 :slotted 伪类，用于解决 Vue3 中 子组件 scoped 样式不能作用于父组件传递过来的插槽元素，原理就是额外绑定上插槽元素标识符属性（如 data-v-child-s）。

4. 子组件插槽的默认填充内容，不属于父组件传入的插槽内容，可以被子组件 scoped 样式作用，不需要使用 :slotted 伪类。
因此插槽默认填充内容不会携带父组件作用域标识符属性，只会携带子组件作用域标识符属性（Vue3 中，会额外带上 -s 后缀的插槽元素标识符属性）。
:::
...
#### ✅不搭配 sass 之类的预处理器
> 在不使用 sass 之类的 CSS 预处理器的前提下，使用 `>>>` 操作符，Vue2 和 Vue3 都是能正确解析转换的。
...
### /deep/ 和 ::v-deep
> 二者都是 `>>>` 的别名，同样能穿透 scoped 限制。相较 `>>>`，对预处理器支持稍好，能被 Vue Loader 正确识别转换。[Vue-loader15](https://vue-loader.vuejs.org/zh/guide/scoped-css.html#%E6%B7%B1%E5%BA%A6%E4%BD%9C%E7%94%A8%E9%80%89%E6%8B%A9%E5%99%A8)官方文档里也明确指出，当 `>>>` 无法正常解析时，可以用 /deep/ 或 ::v-deep 替代。
:::warning
...
#### ✅不搭配 sass 之类的预处理器
> 在不使用 sass 之类的 CSS 预处理器的前提下，使用 `/deep/` 操作符，Vue2 和 Vue3 都是能正确解析转换的。
...
```html [❌Env01、Env02]
<style>
.parent[data-v-parent] { font-size: 16px; }
>>>.child-title[data-v-parent] { color: pink; }
</style>
...
| 测试环境 | 测试结果 | 测试结果说明 |
| --- | --- | --- |
| Env01、Env02 | ❌ | 编译不报错，但不会正确解析 `/deep/` 操作符。 |
| Env03 | ❌ | 编译报错：视为无效 CSS，从生成的 CSS 中移除掉。 |
> Env01 和 Env02 测试结果一致，编译过程不会报错，不会正确解析 `>>>` 操作符。

> Env03 中测试结果不一样，会输出错误提示，被视为无效 CSS，会从生成的 CSS 中移除掉。
