...
你可以根据设备的类型（如屏幕设备、打印机设备）或者特定的特性（如屏幕的宽度）来修改你的页面。

媒体查询的使用方式主要有三种：
  - 方式一（常用）：通过 `@media` 和 `@import` 引入不同的 CSS 文件。
...
在使用媒体查询时，需要指定应用的媒体类型，媒体类型是`可选的`，`默认 all`。
- 常见的媒体类型值如下：
  - `all`（默认值）：适用于所有设备
...
- 被废弃的媒体类型：
  ...
  - 但是在 `Media Queries 4` 中，这些媒体类型已经被废弃，并且不应该被使用
...
通常会将媒体特性描述为一个`表达式`，`每条媒体特性表达式都必须用括号括起来`。
...
> 比如下面的媒体查询，表示：屏幕宽度大于500，小于700的时候，body 背景色为红色。
...
前面编写的 CSS 中，我们经常会使用 `px` 来表示一个长度（大小），如 font-size 设置为18px，width 设置为100px。

`px` 是一个长度（`length`）单位，事实上 CSS 中还有非常多的长度单位，整体可以分成两类：
> 绝对长度单位（Absolute length units）、相对长度单位（Relative length units）

## 绝对单位

绝对单位：

> 它们与其他任何东西都没有关系，通常被认为总是相同的大小，这些值中的大多数在用于打印时比用于屏幕输出时更有用。

> 例如：我们通常在屏幕上不会使用 cm，但会使用 px (像素)。在打印时，cm 这类绝对单位会更适合打印。
...
> 相对长度单位相对于其他一些东西，如：父元素的字体大小，或视图端口的大小。
...
> 像素是**影响显示的基本单位**（如屏幕上看到的画面、一幅图片）。
  
> `pix` 是 `picture` 的常用简写，加上“元素” `element`，就得到 `pixel`。

> 即：pix（picture，图像） + el（element，元素) = pixel（简称 px，像素）。
  
> “像素”表示“图像元素”之意，有时亦被称为 pel（picture element）。
...
- 物理像素（设备像素）
  > 物理像素指的是**设备显示屏上的真实像素**，每个像素的大小是屏幕固有的属性，**显示屏出厂以后就不会改变了**，
  我们在购买显示器或者手机的时候，提到的设备分辨率就是设备像素的大小。
  比如 iPhoneX 的分辨率 1125x2436，指的就是设备像素。
  
- 逻辑像素（设备独立像素）
  > 如果面向开发者我们使用设备像素显示一个 100px 的宽度，那么在不同屏幕上显示效果会是不同的，
  开发者针对不同的屏幕很难进行较好的适配，编写程序必须了解用户的分辨率来进行开发，
  所以在设备像素之上，**操作系统为开发者进行抽象，提供了逻辑像素的概念**。
  比如你购买了一台显示器，在操作系统上是以 1920x1080 设置的显示分辨率，那么无论你购买的是2k、4k的显示器，
  对于开发者来说，都是 1920x1080 的大小，**是操作系统提供的一种逻辑像素，用来调节在不同物理像素设备中的显示效果保持一致的**。
  
- CSS 像素
  > CSS 中经常使用的单位是 px，它在`默认情况下等同于逻辑像素`，毕竟逻辑像素才是面向我们开发者的。
  
我们可以通过 JS 中的 `screen.width` 和 `screen.height` 获取到电脑的逻辑分辨率：

<img src="./assets/image-20220501221106162.png" alt="image-20220501221106162" />	

::: info 案例
假如某台设备 `A` 的屏幕的物理像素是 1920x1080，设备 `A` 屏幕分辨率也是设置的 1920x1080 的逻辑像素，那么 `A` 的设备像素比就是1
（即：1 个逻辑像素在长度上对应 1 个物理像素）。

当在设备 `A` 上浏览某个网页时，该网页有个图片设置的 CSS 像素宽高为 1920x1080 px，
那么该图片在设备 `A` 的屏幕上显示效果是刚好能占满整个屏幕的，
假设该图片原图的像素点是 2560x1440，但由于设备 `A` 的物理像素最多只有 1980x1040 个，就展示不出原图一样的效果。

现在又有一个设备 `B`，它的屏幕的物理像素是 2560x1440，设备 `B` 屏幕分辨率设置的 也是 1920x1080 的逻辑像素，那么 `B` 的设备像素比就是3（即1个逻辑像素在长度上对应3个物理像素）。

当在设备 `B` 上浏览该图片时，由于设备 `B` 的逻辑像素也是 1920x1080，该图片在设备 `B` 的屏幕上显示效果也是刚好能占满整个屏幕的，
但是因为设备 `B` 的物理像素有 2560x1440，所以该图片在设备 `B` 上能显示出原图一样的效果，显示的效果比设备 `A` 更加清晰细腻。

日常开发中，以及设计稿里的都是逻辑像素，如果图片不够清晰，在不能调整图片在页面中显示的区域宽高时，
可以放大图片本身的尺寸（增加像素，一般是保持宽高比按倍数放大），以达到在同一台设备上显示的图片更清晰。

在不能调整物理设备的物理像素时，且不能调整图片的宽高时，可以尝试放大图片，以达到显示效果更清晰。

如果是截图分享海报的这种形式，在截图之前放大海报的尺寸，截图后调回尺寸。
:::
...
::: info
设备像素比（Device Pixel Ratio，简称 DPR）是前端开发中非常重要的一个概念，主要用于描述 设备物理像素 与 CSS 像素 之间的比例关系。
:::
...
在浏览器中，可以通过 window.devicePixelRatio 获取当前屏幕的设备像素比：
```js
...
| 类型                       | 含义                           | 举例                                        |
| ------------------------ | ---------------------------- | ----------------------------------------- |
| **物理像素（Physical Pixel）** | 屏幕上实际存在的发光点，是硬件层面定义的。        | iPhone 13 的屏幕是 1170×2532 个物理像素点           |
| **CSS 像素（CSS Pixel）**    | 浏览器布局使用的逻辑像素单位，是开发者写样式时用的单位。 | CSS 中定义 `width: 100px`，这 100px 指的是 CSS 像素 |


### 设备像素比的由来

在早期（如老款显示器）中：
- 1 个 CSS 像素 = 1 个物理像素，所以 DPR = 1

后来随着“视网膜屏幕”（Retina Display）出现，
> 2010年，iPhone4 问世，不仅仅带来了移动互联网，还带来了 Retina 屏幕，Retina 屏幕翻译为视网膜显示屏，可以为用户带来更好的显示。

<img src="./assets/image-20220501222840086.png" alt="image-20220501222840086" style="zoom:80%;" />


- 屏幕的物理像素变得更密集（分辨率更高），
- 但浏览器仍希望网页内容大小一致，因此会将多个物理像素映射为一个 CSS 像素。

例如：iPhone 的 Retina 屏幕：1 个 CSS 像素 = 2×2 个物理像素 → devicePixelRatio = 2


### 常见设备 DPR 值

| 设备                 | 物理分辨率     | 逻辑分辨率（CSS 像素） | DPR |
| ------------------ | --------- | ------------- | --- |
| 普通电脑显示器            | 1920×1080 | 1920×1080     | 1   |
| MacBook Pro Retina | 2880×1800 | 1440×900      | 2   |
| iPhone 8           | 750×1334  | 375×667       | 2   |
| iPhone 13 Pro      | 1170×2532 | 390×844       | 3   |


### 用途与影响

:::details 1️⃣ 图片与图标清晰度
如果在高 DPR 屏幕上仍使用 1x 图片，就会出现模糊。
...
::: details 2️⃣ Canvas 绘制
Canvas 默认是按 CSS 像素绘制的，如果 DPR>1，不处理会模糊。
...
- 移动端开发目前主要包括三类：
  - 原生 App 开发（iOS、Android、RN、uniapp、Flutter等）
  
  - 小程序开发（原生小程序、uniapp、Taro等）
  
  - Web 页面（移动端的 Web 页面，可以使用浏览器 或 webview 浏览）
...
- 这里有两个概念：
  ...
  -  响应式：会随着屏幕的**实时变动而自动调整**，是一种**实时的自适应**
