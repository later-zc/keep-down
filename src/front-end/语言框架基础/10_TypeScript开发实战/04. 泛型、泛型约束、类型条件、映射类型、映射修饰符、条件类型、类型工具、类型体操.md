...
## `Required<Type>`
与 Partial 相反，Required 工具类型会**将所有属性设为必填**。
这意味着返回的新类型将不允许任何属性为 undefined。
...
## `Readonly<Type>`
`Readonly<Type>` 工具类型会将一个对象的**所有属性设为只读**，意味着这些属性不能再被修改。
...
```
> Readonly 常用于确保对象一旦创建后不可修改，通常用于保证不被意外修改的数据。


## `Record<Keys, Type>`
`Record<Keys, Type>` 工具类型**用于定义一个键值对的对象类型，该对象所有的 key 都是 Keys 类型，所有的 value 都是 Type 类型**。
...
## `Pick<Type, Keys>`
`Pick<Type, Keys>` 工具类型用于从原类型中挑选出特定的属性。
你可以指定一个类型和需要提取的属性名，它会生成一个新类型。
...
## `Omit<Type, Keys>`
`Omit<Type, Keys>` 工具类型与 Pick 相反，它用于从原类型中剔除某些属性。
