...
>- 对于 package.json 等 JSON 文件，Prettier 只遵循 JSON 语法进行缩进和换行，JS/TS 风格规则（semi / singleQuote）不会生效。
...
> 关于 JSON 文件的说明
> - JSON 配置文件，如 `package.json`，`tsconfig.json` 等，不在 Prettier 格式化默认的作用文件类型范围中，所以通常不用担心会被格式化。
> - 对于 package.json 等 JSON 文件，Prettier 只遵循 JSON 语法进行缩进和换行，JS/TS 风格规则（semi / singleQuote）不会生效。
