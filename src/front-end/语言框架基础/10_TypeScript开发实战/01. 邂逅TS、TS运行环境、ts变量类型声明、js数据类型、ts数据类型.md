...
- 现在我们想探究的就是如何在**代码编译期间** **发现代码的错误：**
  ...
  getLength('ab') // 正确调用
  getLength() // 错误的调用（IDE不会报错）
  ```
  实际运行时，会报错：
  ```javascript
  TypeError: Cannot read property 'length' of undefined
  ```
  而 typescript 则可以做到在编写时就能检测到类型错误：
  ```typescript
  function getLength(args: {length: number}) {
    return args.length
  }
  getLength('abc') // 在访问其length属性时，会创建对应的包装类型的对象，然后返回包装对象的length属性
  getLength([1, 2, 3]) // 数组本身也是个对象，也具有length属性
  getLength({}) // IDE会提示报错：类型"{}"的参数不能赋给类型“{length: number}”的参数。参数"{}"中缺少属性"length"，但类型“{length: number}”中需要该属性。
  getLength(123) // IDE会提示报错：类型"number"的参数不能赋给类型“{length: number}”的参数
