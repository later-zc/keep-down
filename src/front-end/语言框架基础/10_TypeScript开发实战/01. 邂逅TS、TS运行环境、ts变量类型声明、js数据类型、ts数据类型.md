...
## 变量的类型声明（类型注解）
- **什么是类型注解？**  
  在 TypeScript 中，变量需要定义数据类型，称为类型注解（Type Annotation）。这样可以帮助 TypeScript 检查代码中的类型错误。
...
- **变量声明格式**  
  ...
  ```
  其中，`变量名`是你要定义的标识符，`数据类型`是这个变量应该存储的类型，`值`是你为这个变量赋予的初始值。
...
- **示例：字符串类型声明**  
  比如我们想声明一个字符串类型的变量 message，可以这样写：
    ...
    - 如果我们给 message 赋值其他类型的值，那么TypeScript 编译器会在编译阶段检测到类型不匹配并提示错误：
      ```typescript
...
- 我们更希望可以**通过 `ts` 本身的特性帮助我们 `推断` 出对应的 `变量类型`**：
  ...
  // 编译器会在编译阶段检就推导出来 'message' 值为 'string' 类型
  ```
