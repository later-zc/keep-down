...
## Object 类型（对象类型）
使用 object 表示通用对象类型，但不包含具体属性信息，本质上也就是一个空对象类型：
  ```typescript
  const myInfo: object = { name: 'later', age: 18 }
  ```

当类型为 Object 时，无法访问或修改对象的属性，所以以下操作会报错：
  ```typescript
  myInfo['name'] = 'later-zc' // [!code error] // 报错
  console.log(myInfo['name']) // [!code error] // 报错
  ```

报错信息如下：

  <img src="./assets/image-20221012225139746.png" alt="image-20221012225139746" style="zoom:100%;" />
...
现在我们可以通过 `Symbol` 来定义同名属性，`Symbol` 是 `ES6` 引入的一种独特且不可重复的值，Symbol 函数返回的是不同的值：
