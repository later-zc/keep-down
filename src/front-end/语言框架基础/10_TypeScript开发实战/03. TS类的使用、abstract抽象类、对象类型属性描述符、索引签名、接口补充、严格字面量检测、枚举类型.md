...
示例：
 ...
 class Person {
   ...
   age: number // age 属性在构造函数中有明确赋值
   sex!: boolean // 特殊语法，跳过初始化检查（带有明确赋值断言的声明还必须具有类型注释）
   _id = 1 // 可省略类型注解，会根据初始值自动推导出类型 
...
- 我们这里来演示一下 `protected` 和 `private`：
  ...
  p1.name // [!code error] // 报错：属性“name”是受保护的，只能在类“Person”及其子类中访问
  const s1 = new Student('later-zc')
  s1.name // [!code error] // 报错：属性“name”是受保护的，只能在类“Person”及其子类中访问
