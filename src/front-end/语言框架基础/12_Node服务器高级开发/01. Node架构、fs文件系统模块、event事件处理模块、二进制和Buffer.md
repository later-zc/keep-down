...
<img src="./assets/image-20230201191455744.png" />
...
也就是说 Node.js 基于 V8 引擎来执行 JS 代码，但是不只有 V8 引擎：
  ...
  > 另外，在 Node.js 中我们也需要进行一些额外的操作，比如：文件系统读写、网络 IO、加密、压缩解压文件等操作。
...
<img src="./assets/image-20230201194251709.png" />



## Node.js 架构

<img src="./assets/image-20230201194519668.png" />

我们来看一个单独的 Node.js 的架构图：
  > 我们编写的 JS 代码会经过 V8 引擎，再通过 Node.js 的 Bindings，将任务放到 Libuv 的事件循环中。

  > libuv（Unicorn Velociraptor — 独角伶盗龙）是使用 C 语言编写的库，libuv 提供了事件循环、文件系统读写、网络 IO、线程池等等内容。
...
Node 也有自己的文件系统操作模块，就是 fs：
  > 借助于 Node 帮我们封装的文件系统，我们可以在任何的操作系统（Windows、MacOS、Linux）上面直接去操作文件。    
...
Node 文件系统的 [API](https://nodejs.org/docs/latest-v16.x/api/fs.html) 非常多：
  > 我们不可能，也没必要一个个去学习。  
...
但是这些 API 大多数都提供三种操作方式：
  ...
  >- 方式三：**异步 Promise 操作文件**（代码不会被阻塞，通过 fs.promises 调用方法操作，会返回一个 Promise，可以通过 then、catch 等方法对结果进行处理）。
...
我们先来看 flag，[flag 的值有很多](https://nodejs.org/dist/latest-v14.x/docs/api/fs.html#fs_file_system_flags)：

  > `w`（默认值）：打开文件写入，文件不存在则创建文件。  
...
我们再来看看编码：
  ...
  > 在简书上一篇关于字符编码的[文章](https://www.jianshu.com/p/899e749be47c)  
  > 目前基本用的都是 UTF-8 编码

文件读取：

  > 如果不填写 encoding，默认返回结果是 `Buffer`，可以通过 `toString()` 转换成字符串。
...
- `fs.readdir()` 或 `fs.readdirSync()` 读取文件夹中的内容

  ```js
  const fs = require('fs')
    
  // 封装一个递归读取目录的工具函数
  function recursionReadDirectory(dir) {
    fs.readdir(dir, { withFileTypes: true }, (err, files) => {
      console.log('files: ', files)
      files.forEach((file) => {
        if (file.isDirectory()) {
          console.log(file.name, '是一个文件夹')
          recursionReadDirectory(`${dir}/${file.name}`)
        } else {
...
**发出事件和监听事件都是通过 EventEmitter 类来完成的**，它们都属于 events 对象。
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      emitter.emit('later') // 监听器被移除了，不会再执行了
    }, 1000)
  }, 2000)
...
EventEmitter 的实例有一些属性，可以记录一些信息：
  ...
  // 最大监听个数:  10
  // aaa事件的监听器个数:  3
  // aaa事件的监听器函数:  [[Function (anonymous)], [Function (anonymous)], [Function (anonymous)]]
  ```
...
但是对于服务器来说是不一样的：
  ...
  > 比如我们需要读取的是一张图片数据（二进制），再通过某些手段对图片数据进行二次的处理（裁剪、格式转换、旋转、添加滤镜），Node 中有一个 Sharp 的库，就是读取图片或传入图片的 Buffer 对其再进行处理。
...
我们会发现，对于前端开发来说，通常很少会和二进制直接打交道，但是服务器端为了实现很多的功能，必须直接去操作其二进制的数据。

所以 Node 为了可以方便开发者完成更多功能，提供给了我们一个全局类 Buffer。
...
为什么是 8 位呢？
  > 在计算机中，我们很少会直接操作一位二进制，因为一位二进制存储的数据是非常有限的。

  > 所以通常会将**8位二进制合在一起作为一个单元，这个单元称之为一个字节(byte)**。  
  > 也就是说 `1byte = 8bit`，`1kb = 1024byte`，`1M = 1024kb`。
  
  > 比如：很多编程语言中的 int 类型是4个字节，long 类型是8个字节。  
  > 比如：TCP 传输的是字节流，在写入和读取时都需要说明字节的个数。  
  > 比如：RGB 的最大值分别都是255，所以在计算机中本质上都是用一个字节存储的（因为`1`个字节对应的8位二进制最大值 => `1111 1111`，转成十进制就是`255`）。
...
<img src="./assets/image-20230203154157581.png" />
...
<img src="./assets/image-20230203170345750.png" />
...
事实上我们创建 Buffer 时，并不会频繁的向操作系统申请内存，它会默认先申请一个 `8*1024` 个字节大小的内存，也就是 `8kb`。
  ...
  <img src="./assets/image-20230203194037763.png" />
...
假如我们调用 Buffer.from 申请 Buffer：
  ...
  <img src="./assets/image-20230203194202950.png" />



## fromString 的源码

<img src="./assets/image-20230203194246080.png" />
...
接着我们查看 fromStringFast：
  ...
  ![](./assets/image-20230203194353057.png)
  
  ![](./assets/image-20230203194413503.png)
