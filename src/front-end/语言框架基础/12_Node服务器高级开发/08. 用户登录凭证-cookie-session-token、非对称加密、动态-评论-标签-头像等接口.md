...
Cookie (小饼干/曲奇饼)。类型为 '小型文本文件'，某些网站**为了辨别用户身份而存储在用户本地终端（Client Side）上的数据**。
> 浏览器会在特定的情况下携带上 cookie 来发送请求，我们可以通过 cookie 来获取一些信息。

Cookie 总是保存在客户端中，根据其客户端中的存储位置，分为两种：
...
如何判断一个 cookie 是内存 cookie 还是硬盘 cookie 呢？
  > - 没有设置过期时间，默认情况下是内存 cookie，在关闭浏览器时会自动删除。
  > - 有设置过期时间，并且过期时间不为 0 或者负数的 cookie，是硬盘 cookie，只能手动删除或过期自动删除。

::: warning
**会话 cookie 只有当浏览器所有标签页都关闭时，才会被清理**，只关闭浏览器标签页，而不关闭整个浏览器，是无法被清理的。一些浏览器重启时会使用会话恢复。这可能导致会话 cookie 无限延长。。
...
cookie 的生命周期：
  > 默认情况下的 cookie 是内存 cookie，也就是在浏览器所有标签页关闭时会自动被删除。

  > expires 或 max-age 用于设置过期的时间：
  >   - `expires`：设置的是 Date.toUTCString()，设置格式是 expires=date-in-GMTString-format。
  >   - `max-age`：设置到期的时间（单位：s），设置格式是 max-age=max-age-in-seconds (例如一年为60 * 60 * 24 * 365)。

cookie 的作用域（允许 cookie 发送给哪些 url）：
> `Domain` 用于指定哪些主机可以接受 cookie：
> - 如果不指定，那么默认是 origin（当前创建 cookie 的 host），不包含子域名。
> - 如果指定 Domain，则包含子域名。
> - 例如：如果设置 Domain=mozilla.org，则 Cookie 也包含在子域名中（如 developer.mozilla.org）。

> `Path` 用于指定主机下哪些路径可以接受 cookie（只有与创建 cookie 的页面在同一个目录或子目录下的网页才可以访问） 
...
这个 cookie 会在浏览器所有标签页都关闭时，才被删除掉：
...
设置带过期时间的 cookie（max-age 默认单位是秒）：
...
典型的 session 登陆/验证流程：
  <img src="./assets/image-20230423151605460.png" alt="image-20230423151605460" />
...
在 Koa 中，我们可以借助于 [koa-session](https://github.com/koajs/session) 中间件来实现 session 认证：

```js
// session 基于 cookie 的基础上，进行加密算法，签名认证
// 从而有效防止伪造和篡改 cookie
...
const session = koaSession(
  {
    key: 'sessionid', // cookie 的 key
    signed: true,     // 是否使用 signed 签名认证，防止数据被篡改
    maxAge: 5 * 1000, // 过期时间（单位s）
    httpOnly: true,   // 不允许 js 获取 cookie
...
## session的缺点，认识令牌（凭证）token

cookie 和 session 的方式有很多的缺点：
  > 1. **cookie 会被附加在每个 HTTP 请求中**，所以**无形中增加了流量消耗**（事实上某些请求是不需要的）》
  > 2. Cookie 是**明文传递的**，所以**存在安全性的问题**。
  > 3. Cookie 的**大小限制是 4KB**，对于复杂的需求来说是不够的。 
  > 4. 对于**浏览器之外的其他客户端**（比如 iOS、Android），**必须手动设置 cookie 和 session**。 
  > 5. 对于**分布式系统和服务器集群**中**如何保证其他系统也可以正确的解析 session** ？
  > 通常服务端是集群，而用户请求过来会走一次负载均衡，不一定打到哪台机器上。那一旦用户后续接口请求到的机器和他登录请求的机器不一致，或者登录请求的机器宕机了，session 不就失效了吗？
  > - 这个问题现在有几种解决方式： 
  >   - 方式一：从存储角度，**集中存储 session**。如果我们用独立的 Redis 或普通数据库，就可以把 session 都存到一个库里。
  >   - 方式二：从分布角度，**让相同 IP 的请求在负载均衡时都打到同一台机器上**。以 nginx 为例，可以配置 ip_hash 来实现。
  >   - 通常采用第一种方式，第二种方式相当于阉割了负载均衡，且仍没有解决用户请求的机器宕机问题。

所以，在目前的前后端分离的开发过程中，使用 token 来进行身份验证的是最多的情况：
> token 译为令牌，即在验证了用户账号和密码正确的前提下，给用户颁发一个令牌。  
>  该令牌作为后续用户访问一些接口或资源的凭证，根据这个凭证来判断用户是否有权限来访问。

所以  token 的使用应该分成两个重要的步骤：
> 1. 生成 token：登录的时候，颁发 token，
> 2. 验证 token：访问某些资源或接口时，验证 token。



## JWT 实现 Token 机制

![img](./assets/image-20230423151411603.png)

![img](./assets/image-20230423152107707.png)

JWT（JSON Web Token） 生成的 Token 由三部分组成：

- header：
  > - alg：采用的加密算法，默认是 HMAC SHA256（HS256），采用对称加密（即同一个密钥进行加密和解密）。
...
前面我们说过，`HS256` 对称加密算法一但密钥暴露就是非常危险的事情：
...
我们可以使用 `openssl` 来生成一对私钥和公钥：
  ...
  <img src="./assets/image-20230423172138378.png" alt="image-20230423172138378" />
