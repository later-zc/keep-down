...
> 最终，`noscript` 元素出现，被用于**给不支持 JS 的浏览器提供替代内容**。

下面的情况下，浏览器将显示包含在 `<noscript>` 中的内容: 

> **浏览器不支持脚本**、**浏览器对脚本的支持被关闭**、...

<img src="./assets/image-20220507185043030.png" />
...
<img src="./assets/image-20220507192033551.png" />
...
语句（Statements）是**向浏览器发出的指令**，通常表达一个**操作或者行为**（Action）。
...
> JS 将**换行符**理解成“**隐式**”的**分号**，这也被称之为**自动插入分号**（automatic semicolon）。
