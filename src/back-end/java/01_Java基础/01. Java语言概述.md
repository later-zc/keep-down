...
② **编译**：使用 javac 命令将 .java 文件编译成 .class 字节码文件（一个或多个）。 
...
- ① **打开命令行终端**
  ...
  > 使用资源管理器打开 D:\JavaSE\chapter01 目录，在地址栏输入 cmd 并回车，打开命令行窗口：
...
- 举例1：
  ...
  1 error
  ```
  - **声明为 public 的类必须与文件名完全一致（区分大小写），否则编译失败**
...
- 解决方案二：
  ...
  >   ```
  >   <img src="./assets/image-20231210190709987.png" />
  >
