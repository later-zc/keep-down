...
| 创建空文件 | `type nul > 文件名`   | `touch 文件名` | CMD 中可用 `echo.` 重定向生成空文件 |
...
| 编辑文件内容（终端内） | `copy con 文件名`<br>或 `notepad 文件名`(推荐) | `nano 文件名` 或 `vim 文件名` | 文件若不存在，则会创建。<br>copy con 是直接覆盖，无法基于文件内容编辑，每次需要重新输入。<br>nano 更适合新手；vim 功能强大但学习成本较高 |
