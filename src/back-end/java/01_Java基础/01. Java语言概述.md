...
| 删除空目录 | `rd 目录名` | `rmdir 目录名` 或 `rm -d 目录名` | 仅能删除空目录 |
| 强制删除非空目录 | `rd /s /q 目录名` | `rm -rf 目录名` | **危险操作**，强制递归删除所有内容 |
| 创建空文件 | `echo. > 文件名` | `touch 文件名` | CMD 中可用 `echo.` 重定向生成空文件 |
...
写完整文件后缀，可被大多数系统/编辑器自动识别，更容易被其他人或工具识别用途。
:::

| 操作 | Windows CMD | macOS Terminal | 说明 |
|-|-|-|-|
| 查看文件内容 | `type 文件名` | `cat 文件名` | 适用于小文件；内容较多时推荐分页查看 |
| 分页查看文件 | `more 文件名` | `more 文件名` 或 `less 文件名` | macOS 推荐使用 `less`，支持上下滚动 |
| 查看前几行内容 | — | `head -n 10 文件名` | 查看文件前 10 行 |
| 查看后几行内容 | — | `tail -n 10 文件名` | 查看文件后 10 行 |
| 编辑文件内容（终端内） | `copy con 文件名` | `nano 文件名` 或 `vim 文件名` | 文件若不存在，则会创建。nano 更适合新手；vim 功能强大但学习成本较高 |
| 编辑文件内容（图形） | `notepad 文件名` 或 `start 文件名` | `open 文件名` | |
...
<img src="./assets/image-20221027112824724.png" />
...
> ② **编译**：通过 javac.exe 命令对该 **.java 源文件进行编译**，**生成一个或多个字节码文件**；  
> ③ **运行**：通过 java.exe 命令**运行生成的 .class 字节码文件**
...
2. 用记事本或 editplus 等文本编辑器打开（记事本没有关键字颜色标识，不利于初学者学习）。
...
## 编译 Java 源代码
- 在 DOS 命令行中，**使用 javac 命令对 .java 源文件进行编译**，命令格式：
  ```shell
  javac Java源文件名.后缀名java
  ```

- 先使用文件资源管理器打开 D:\JavaSE\chapter01 目录，然后在地址栏输入 cmd 并回车：
...
- 编译成功后，命令行没有任何提示，打开 D:\JavaSE\chapter01 目录，发现产生了一个新的文件 `HelloChina.class`，该文件就是编译后的`字节码文件`，是 Java 的`可运行文件`，有了字节码文件就可以运行程序了。


## 运行字节码文件
- **使用 java 命令执行字节码文件，格式如下：**
  ```bash
  java 主类名字 # 如 HelloChina
  ```
  ::: warning
  Java 运行命令：传入的是类名，不是文件名或 .class 文件名。
  > 示例：
  ```shell
  # 正确示范
  java HelloChina
...
## 05. jdk11新增特性：java命令自动编译并执行java源文件
